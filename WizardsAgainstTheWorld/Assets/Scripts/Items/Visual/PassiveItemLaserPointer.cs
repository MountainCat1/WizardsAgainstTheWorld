...
namespace Items.Visual
{
    public class PassiveItemLaserPointer : MonoBehaviour
    {
        ...
        private void Awake()
        {
            item.Activated += OnItemUsed;
            item.Deactivated += OnItemUnUsed;

            ResetLaser();
        }

        private void OnItemUnUsed()
        {
            if (item.Creature != null)
            {
                UnregisterCreature(item.Creature);
            }
